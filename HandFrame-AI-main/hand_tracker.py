...
class HandTracker:
    def __init__(self, max_hands=2, det_conf=0.6, track_conf=0.6,
                 ...
                 model_path=None):
        # MediaPipe 1.x removed the legacy `mp.solutions` API; the supported
        # interface is the new Tasks API (HandLandmarker). It requires a
        # hand_landmarker.task model file, bundled alongside this project.
    ...
    def process(self, frame_bgr, t=None):
        ...
        if result.hand_landmarks and result.handedness:
            for lm_set, handedness in zip(result.hand_landmarks, result.handedness):
                ...
                if label not in self.hands:
                    label = "Left" if "Left" in str(label) else "Right"
                pts = np.array(
                    [[lm.x * w, lm.y * h] for lm in lm_set],
                    dtype=np.float64,
                )
