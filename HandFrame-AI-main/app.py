...
def main():
    ...
    try:
        while True:
            ...
            try:
                quit_requested = _process_frame(
                    args, frame, tracker, plane, quad_smoother,
                    engine, state, fps_meter, window_name,
                )
            except Exception as exc:  # noqa: BLE001
                print(f"[frame] skipped one bad frame: {exc}")
...
def _process_frame(args, frame, tracker, plane, quad_smoother,
                   engine, state, fps_meter, window_name):
    """
    Handles one video frame: mirror, hand tracking, gesture/pinch logic,
    AI result polling, plane rendering, HUD, and keyboard handling.
    ...
    # Runs every frame (even when the gesture is momentarily lost) so
    # a finished job is consumed instead of being stranded in the queue.
