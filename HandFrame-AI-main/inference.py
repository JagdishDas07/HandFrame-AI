...
class AsyncFluxEngine:
    ...
    def _get_face_mask(cls, img):
        ...
        if cls._face_cascade is None:
            # OpenCV 5.x no longer bundles the haarcascade XML files in the
            # wheel, so the file may be missing. Guard against that so the
            # face-aware styles degrade gracefully to a center-frame guess
            # instead of crashing.
